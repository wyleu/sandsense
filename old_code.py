...
def format_time(struct_time=None):
    """
    Returns a human-readable time string from a time.struct_time object.
    If no argument is given, uses current local time.
    """
    ...
    return (f"{weekdays[struct_time.tm_wday]} {struct_time.tm_mday} {months[struct_time.tm_mon]} {struct_time.tm_year} "
            f"{struct_time.tm_hour:02d}:{struct_time.tm_min:02d}:{struct_time.tm_sec:02d}")

def format_time_iso(struct_time=None):
    if struct_time is None:
        struct_time = time.localtime()
    return (f"{struct_time.tm_year:04d}-{struct_time.tm_mon:02d}-{struct_time.tm_mday:02d} "
            f"{struct_time.tm_hour:02d}:{struct_time.tm_min:02d}:{struct_time.tm_sec:02d}")
