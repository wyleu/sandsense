...
from sand_net import connect_wifi, start_mdns, hostname_from_cfg, sync_time, last_ssid
from sand_status import from_sandsense

# Requires: adafruit_httpserver in /lib
try:
...
_status_server = None
_tick_ws = None          # ← single live WebSocket client
...
def _status_payload():
    ...
    try:
        ...
        pass
    try:
        ssid = wifi.radio.ap_info.ssid if wifi.radio.ap_info else None
    except Exception:
        pass
...
def start_status_server():
    """Call once after successful WiFi connect."""
    ...
    try:
        ...
        def status_route(request: Request):
            ...
            # CORS so sandbells2.local can fetch from the Pico
            return Response(
                request,
                body=body,
...
#     )
    ...
    msg = (f"{format_time()} {elapsed:.1f} s {tick_count} tks {tpm:.2f}   (aim 48.00) {'ON' if PLL_ENABLED else 'OFF'} {pll_locked}  misses={consecutive_misses} {adc_avg:.0f} / {high_thresh:.0f} / {low_thresh:.0f} {temp_str}")
...
class StatusHttpTask(AppTask):
    """Non-blocking poll of the HTTP server (messages only)."""
    name = "status_http"
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                # avoid killing the gather on one bad request
                print("status poll:", e)
            await asyncio.sleep(0.05)  # ~20 Hz poll; ADC/laser keep running

class WifiWatchTask(AppTask):
    name = "wifi_watch"
    async def run(self):
        while True:
...
def _wifi_candidates():
    ...
    if ssid:
        return [{"ssid": ssid, "password": password, "priority": 1}]
    return []

def setup_wifi():
    if not connect_wifi(cfg):
        return
    host = hostname_from_cfg(cfg)
    start_mdns(host)
    pool = adafruit_connection_manager.get_radio_socketpool(wifi.radio)
    sync_time(cfg, pool)
    start_status_server()
