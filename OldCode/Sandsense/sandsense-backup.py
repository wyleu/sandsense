    ...
    def run(self):
        ...
        while not self.done_event.is_set():
            try:
                ...
                if tick % beats_per_minute == 0 and self.tick_LED:          # Clock Minute
...
class LDR():
    ...
    def take_reading(self,times = 5, sleep_time = 0.001):
        ...
        return (dark_average, light_average, dark_min, dark_max, threshold, light_min, light_max,)
...
class MeasureThread(RGBEncoderThread):
    ...
    def run_calibrate(self):
        print('dark_average, light_average, dark_min, dark_max, threshold, light_min, light_max,')
        logging.info(f'FL {self.LDR_front_left.take_reading()}')
        logging.info(f'FR {self.LDR_front_right.take_reading()}')
        logging.info(f'SL {self.LDR_left.take_reading()}')
        logging.info(f'SR {self.LDR_right.take_reading()}')  
...
def main():
    ...
    Measure_helper = MeasureThread(2, measure_queue, response_queue2, done_event)
    ...
    response_queue1.put('Start_Measure')

    try:
        while True:
            # Check responses from helper threads
            try:
                response1 = response_queue1.get_nowait()  # No_wait for measure loop
                if response1 == 'Start_Calibrate':
                    measure_queue.put('Run_Calibrate')
                    
                elif response1 == 'Start_Measure':
                    logging.info(f' START MEASURE Response 1 {response1}') 
                    measure_queue.put('Start_Measure')
                    
                elif response1.find('Distance') == 0 :
            ...
            # logging.info(f'Response Q1: {response_queue1.qsize()} Response Q2: {response_queue2.qsize()}   Measure Q: {measure_queue.qsize()}')
