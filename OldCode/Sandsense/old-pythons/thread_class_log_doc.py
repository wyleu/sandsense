...
class Watch():
    ...
    def test(self, times = 10):
        sleep_time_list = [0.05, 0.01, 0.005, 0.001]
        ...
        for sleep_time in sleep_time_list: 
            ...
            logging.info(f'{self.name} | {dark_min} {dark_total} {dark_max}  > {round(dark_total + ((light_total-dark_total))/2) } < {light_min}  {light_total}  {light_max}  |  {light_max-dark_min}  {round(light_total-dark_total,2)}  {light_min-dark_max}  | {sleep_time}')
...
class CalibrateThread(RGBEncoderThread):
    ...
    def setup_watch(self):
        ...
        self.watch_front_right = Watch('FRONT RIGHT', self.laser_front, self.LDR_front_right)
        self.watch_front_left = Watch('FRONT LEFT ', self.laser_front, self.LDR_front_left)
...
if __name__ == "__main__":
    ...
    calibration_runner = threading.Thread(target=Calibrate_helper.calibrate, name=f"CalibrateThread-{2}")
